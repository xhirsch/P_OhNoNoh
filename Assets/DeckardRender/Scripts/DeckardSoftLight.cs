...
public class DeckardSoftLight : MonoBehaviour
{
    ...
    public Vector2 lightSize = new Vector2(0.05f, 0.05f);
    [HideInInspector]
    public Light lightL;
    ...
    void Start()
    {
        ...
        lightPos = gameObject.transform.localPosition;
        lightRot = gameObject.transform.localRotation;
        if (AddLightMesh)
        {
            if (mRenderer == null)
            {
    ...
    private void Update()
    {
        ...
        shadowBias = lightL.shadowBias;
        lookAtVec = transform.TransformPoint(Vector3.forward * focusDistance);
