...
namespace DeckardRender
{
    ...
    public class DeckardRender : MonoBehaviour
    {
        ...
        public bool safeRenderMode = false;
        public int safeCounter = 0;
        public bool useRangeRendering = false;
        public int rangeMin = 0;
        public int rangeMax = 0;
        public bool numerationsFrom0 = true;
        public float defaultLightSize = 0.02f;
        public FilmTransferStyle filmTransferStyle;
        ...
        private void FixSkinIssues()
        {
            ...
            if (skinnedMeshes.Length > 0)
            {
                ...
                for (int i = 0; i < skinnedMeshes.Length; i++)
                {
                    ...
                    skinnedMeshes[i].allowOcclusionWhenDynamic = false;
                    skinnedMeshes[i].skinnedMotionVectors = true;
        ...
        private void FixNoSoftLights()
        {
            Light[] lights = FindObjectsOfType<Light>();
            if (lights.Length > 0)
            {
                Debug.Log("found lights" + lights.Length);
                for (int i = 0; i < lights.Length; i++)
                {
                    DeckardSoftLight dsl = lights[i].gameObject.GetComponent<DeckardSoftLight>();
                    if (dsl == null)
                    {
                        if (lights[i].type == LightType.Area)
                        {
                            return;
                        }
                        else if ((lights[i].type != LightType.Directional) && usingHDRP)
                            dsl = lights[i].gameObject.AddComponent<DeckardSoftLight>();
        ...
        private void FixNoTimelineAssigned()
        {
            PlayableDirector[] timeLines = FindObjectsOfType<PlayableDirector>();
            if (timeLines.Length > 0 && timeline == null)
            {
                for (int i = 0; i < timeLines.Length; i++)
                {
                    if (timeLines[i].isActiveAndEnabled)
                    {
        ...
        public void Start()
        {
            ...
            else
            {
                if (EditorApplication.isPlaying && captureOnPlay)
                {
                    ...
                    renderQuality = 1;
                    FixSkinIssues();
                    FixNoTimelineAssigned();
                    if (defaultLightSize != 0f)
                        FixNoSoftLights();
                    if (useRangeRendering)
                    {
                        timeline.initialTime = (rangeMin * 1f) / (FPS * 1f);
                    }
        ...
        public void SkinMotionBlur()
        {
            if (timeline != null && UnityEditor.EditorApplication.isPlaying && cinemachineCut == false)
            {
                ...
                {
                    timeline.time = Time.time + (float)timeline.initialTime;
                    timeline.Evaluate();
                }
            }
        ...
        public void Update()
        {
            ...
            if (!captureAudio)
            {
                ...
                if (filmTransferStyle != null)
                {
                    finalPassMat.SetFloat("_saturation1", filmTransferStyle.saturation);
                    finalPassMat.SetFloat("_Size2", filmTransferStyle.filmScattering);
                    finalPassMat.SetFloat("_noiseScale", filmTransferStyle.grainScale);
                    finalPassMat.SetVector("_ContrVec", new Vector4(filmTransferStyle.RContrast, filmTransferStyle.GContrast, filmTransferStyle.BContrast, 0));
                    finalPassMat.SetVector("_CMidPointVec", new Vector4(filmTransferStyle.RMidpoint, filmTransferStyle.GMidpoint, filmTransferStyle.BMidpoint, 0));
                    finalPassMat.SetFloat("_Size", filmTransferStyle.filmScattering);
                    finalPassMat.SetInt("_Directions", filmTransferStyle.directions);
                    finalPassMat.SetFloat("_HalationSize", filmTransferStyle.halationSize);
                    finalPassMat.SetFloat("_NoiseAmount", filmTransferStyle.NoiseAmount);
                    finalPassMat.SetVector("_NoiseResponse", filmTransferStyle.noiseResponse);

                }

                else
                {
                    finalPassMat.SetFloat("_saturation1", 1f);
                    finalPassMat.SetFloat("_Size2", 0f);
                    finalPassMat.SetFloat("_noiseScale", 0.8f);
                    finalPassMat.SetVector("_ContrVec", new Vector4(1f, 1f, 1f, 0));
                    finalPassMat.SetVector("_CMidPointVec", new Vector4(0.5f, 0.5f, 0.5f, 0));
                    finalPassMat.SetFloat("_Size", 1f);
                    finalPassMat.SetInt("_Directions", 1);
                    finalPassMat.SetFloat("_HalationSize", 0f);
                    finalPassMat.SetFloat("_NoiseAmount", 0f);
                    finalPassMat.SetVector("_NoiseResponse", new Vector4(0f, 0f, 0f, 0f));
                }
            ...
            if (active)
            {
                ...
                for (int val = 0; val < finalSteps * 2f; val++)
                {
                    ...
                    foreach (DeckardSoftLight l in lights)
                    {
                        ...
                        {
                            l.transform.localRotation = l.lightRot;
                            ...
                            l.lightL.shadowBias = l.deckardShadowBias;
                            l.lightL.shadowNormalBias = l.deckardNormalBias;
                        }

                        if (l.lightL.type == LightType.Spot)
                        {
                            l.transform.localPosition = l.lightPos;
                            l.transform.localRotation = l.lightRot;
                            Vector3 areaLightNormalized = (new Vector3(UnityEngine.Random.Range(-1f, 1f), UnityEngine.Random.Range(-1f, 1f), 0f));
                            Vector3 areaLightVec = (new Vector3(areaLightNormalized.x * (l.lightSize.x / 2), areaLightNormalized.y * (l.lightSize.y / 2), 0f));
                            l.transform.Translate(areaLightVec, Space.Self);

                            l.transform.LookAt(l.lookAtVec);

                            if (l.lightTexture != null)
                            {
                                Color col = l.lightColor;
                                //      Debug.Log((areaLightNormalized.x * 0.5f + 0.5f) + "   " + (areaLightNormalized.y * 0.5f + 0.5f));
                                try
                                {
                                    col = l.lightTexture.GetPixelBilinear((areaLightNormalized.x * 0.5f + 0.5f), (areaLightNormalized.y * 0.5f + 0.5f), l.mipLevel);
                                }
                                catch (UnityException e)
                                {
                                    if (e.Message.StartsWith("Texture '" + l.lightTexture.name + "' is not readable"))
                                    {
                                        Debug.LogError("Please enable read/write on texture [" + l.lightTexture.name + "]");
                                    }
                                }
                                l.lightL.color = col;
                            }
                ...
                foreach (DeckardSoftLight l in lights)
                {
                    ...
                    {
                        l.transform.localRotation = l.lightRot;
                        l.lightL.shadowBias = l.shadowBias;
                        l.lightL.shadowNormalBias = l.normalBias;
                    }
        ...
        private void OptimizedRendering()
        {
            ...
            if (active)
            {
                ...
                if (curentOptimizedSteps == 0)
                {
                    Graphics.Blit(rt2, bufferPass);
                    ReflectionProbeOptimizer[] rProbes = FindObjectsOfType<ReflectionProbeOptimizer>();
                    foreach (ReflectionProbeOptimizer ro in rProbes)
                    {
                        ro.RenderProbe();
        ...
        private void SafeRendering()
        {
            ...
            if (active)
            {
                ...
                if (active)
                {
                    ...
                    //Graphics.Blit(rt, rt2, multipassMat);
                    if (curentOptimizedSteps >= finalSteps * 2)
                    {
                ...
                if (EditorApplication.isPlaying && captureOnPlay) RenderSafeVideo();
        ...
        public void RenderVideo()
        {
...

            {
                ...
                {
                    if ((frame - 25) >= renderFromFrame && safeRenderMode)
                    {
                        if (curentOptimizedSteps >= finalSteps * 2)
                            SaveScreenshotVideo();
                    }
                    else SaveScreenshotVideo();
                }
        ...
        public void RenderSafeVideo()
        {
            ...
            if ((Time.frameCount - StartFrame) < (NumberOfFramesToRender * (finalSteps / batchGroups) * 2) + 25)

            {
                ...
                if ((Time.frameCount - StartFrame) > 0)
                {
                    ...
                    {
                        Time.captureDeltaTime = 1f / (FPS * (finalSteps * 4f)) * batchGroups;
                        //Debug.Log(Time.time);

                        if (curentOptimizedSteps >= (finalSteps) * 2)
                        {
                            Time.captureDeltaTime = 1f / (FPS * 2);
                ...
                remainingTime = Time.realtimeSinceStartup / (Time.frameCount - StartFrame) * (((NumberOfFramesToRender * finalSteps * 2) / batchGroups) - (Time.frameCount - StartFrame));
                //         remainingTime = Time.deltaTime * (NumberOfFramesToRender - safeCounter) * ((finalSteps * 2) / batchGroups) * ((finalSteps * 2) / batchGroups);
                //    remainingTime = Time.deltaTime * (NumberOfFramesToRender - safeCounter) * batchGroups * finalSteps; 
                //       Debug.Log(remainingTime);
                minutesRemain = (int)remainingTime / 60;
                secondsRemain = (int)remainingTime % 60;
#if UNITY_EDITOR
                if (UnityEditor.EditorUtility.DisplayCancelableProgressBar("Rendering frame " + (safeCounter) + "/" + NumberOfFramesToRender, " Remaining time: " + minutesRemain + " min. " + secondsRemain + " sec.", (float)(Time.frameCount - StartFrame - 25) / (float)((NumberOfFramesToRender * (finalSteps * 2)) / batchGroups)))
                {
                    ClearBar();
                    QuitEditor();
                }
            ...
            else
            {
                ...
                if (encodeToVideo)
                {
                    ...
                    //     else System.Diagnostics.Process.Start(ffmpegPath + "ffmpeg", " -f image2" + " -framerate " + FPS + " -i \"" + fullPath + _prefix + "%05d" + formatString + " -r " + FPS + " -c:v prores_ks -profile:v 3 \"" + fullPath + Folder + ".mov\"");
        ...
        public void SaveScreenshotVideo()
        {
            ...
            if (ImageFormatType == VRFormatList.JPG)
            {
                ...
                {
                    if (!numerationsFrom0)
                        filePathh = string.Format(fullPath + _prefix + "{1:D05}.jpg", Folder, safeCounter + rangeMin);
                    else filePathh = string.Format(fullPath + _prefix + "{1:D05}.jpg", Folder, safeCounter);
                }
                formatString = ".jpg\"";
                //    print(filePathh);
            ...
            else if (ImageFormatType == VRFormatList.PNG)
            {
                ...
                {
                    if (!numerationsFrom0)
                        filePathh = string.Format(fullPath + _prefix + "{1:D05}.png", Folder, safeCounter + rangeMin);
                    else filePathh = string.Format(fullPath + _prefix + "{1:D05}.png", Folder, safeCounter);
                }
                formatString = ".png\"";
                File.WriteAllBytes(filePathh, bytes);
            }
            ...
            else if (ImageFormatType == VRFormatList.EXR_HDRI)
            {
                ...
                {
                    if (!numerationsFrom0)
                        filePathh = string.Format(fullPath + _prefix + "{1:D05}.exr", Folder, safeCounter + rangeMin);
                    else filePathh = string.Format(fullPath + _prefix + "{1:D05}.exr", Folder, safeCounter);
                }
                formatString = ".exr\"";
                File.WriteAllBytes(filePathh, bytes);
            }
        ...
        public void LoadLUT()
        {
            ...
            finalPassMat.SetTexture("_dLUT", filmResponseTex);
