...
namespace DeckardRender
{
    ...
    public class DeckardSoftLightEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (VRP.lightL.type == LightType.Spot)
            {
                VRP.lightTexture = EditorGUILayout.ObjectField("Chromatic abberation Texture", VRP.lightTexture, typeof(Texture2D), true) as Texture2D;
                VRP.focusDistance = EditorGUILayout.FloatField("Focus Distance", VRP.focusDistance);
                VRP.mipLevel = EditorGUILayout.IntField("Cookie Blurring", VRP.mipLevel);
            }

            if (VRP.lightL.type == LightType.Point)
            {
                VRP.lightTexture = EditorGUILayout.ObjectField("Chromatic abberation Texture", VRP.lightTexture, typeof(Texture2D), true) as Texture2D;
           //     VRP.mipLevel = EditorGUILayout.IntField("Cookie Blurring", VRP.mipLevel);
            }
