...
# endif
            ...
            if (VRP.roll1)
            {
                ...
                {
                    VRP.customFilmSize = EditorGUILayout.FloatField("Custom sensor size", VRP.customFilmSize);
                    VRP.LoadFilmSize();
                }
                ...
                VRP.noise = EditorGUILayout.Slider("Film Simulation", VRP.noise, -0.5f, 0f);
                ...
                VRP.filmTransferStyle = EditorGUILayout.ObjectField("Film Transfer Style", VRP.filmTransferStyle, typeof(FilmTransferStyle), true) as FilmTransferStyle;
            }
            ...
            if (VRP.renderQualitySettings)
            {
               // VRP.optimizedSpeedRendering = EditorGUILayout.Toggle(new GUIContent("Responsive Rendering Mode", "This instructs Deckard to use Responsive Rendering Mode. This mode updates rendering slower than default rendering mode, but it prevents editor from having a slow response. Experimental Feature."), VRP.optimizedSpeedRendering);
                ...
                VRP.safeRenderMode = EditorGUILayout.Toggle(new GUIContent("Safe Render Mode", "Safe Rendering Mode - in some cases, standard rendering mode can produce problems with third party plugins, cinemachine, crashes with HDRP Raytrace rendering. This mode should fix eventual problems of this kind, and conform more to Unity workflows. It has some expense on rendering speed - it will reduce rendering speed for simple scenes, while making Deckard more responsive when rendering complex graphics. In some cases it will drastically reduce VRAM usage. It also reduces noise with Raytrace"), VRP.safeRenderMode);
                if (!VRP.progressiveRendering) {
                    ...
                    VRP.editorQuality = EditorGUILayout.IntSlider(new GUIContent("Editor quality", "This value defines how many times we should sample Syntetic Aperture and Motion Blur while using Unity Editor in Edit mode. Higher values give overall better quality, but slower rendering speed. Default is 10"), VRP.editorQuality, 1, 80);
            ...
            if (VRP.animationSettings)
            {
                ...
                VRP.useRangeRendering = EditorGUILayout.Toggle(new GUIContent("Render Timeline Range", "Use precize timing for Physics and particles"), VRP.useRangeRendering);
                if (VRP.useRangeRendering)
                {
                    VRP.numerationsFrom0 = EditorGUILayout.Toggle(new GUIContent("Render Timeline Range", "Use precize timing for Physics and particles"), VRP.numerationsFrom0);
                    VRP.rangeMin = EditorGUILayout.IntField(new GUIContent("RAnge Start", "Resume rendering from frame. This has to be used as resume option after crash or user cancel"), VRP.rangeMin);
                }
